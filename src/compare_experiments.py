...
experiment = mlflow.get_experiment_by_name("smartphone-stress-impact-prediction")
...
for i, row in runs.head(5).iterrows():
    print(f"\nRun: {row['run_id'][:8]}...")
    print(f"  Model:    {row['params.model']}")
...
print("=" * 80)
summary = runs.groupby("params.model")["metrics.f1_score"].agg(["mean", "max", "count"])
